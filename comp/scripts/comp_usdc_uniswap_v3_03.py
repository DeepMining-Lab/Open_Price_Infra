...
DEX_VERSION             = "v3"
BASE_TOKEN_ADDRESS      = TOKEN1_ADDRESS   # COMP
...
def decode_swap_event(data_hex):
    try:
        ...
        return usdc_amount, comp_amount, sqrtPriceX96, liquidity, tick, amount0, amount1
...
def get_slip_10k(web3, block_number, price_usdc_per_comp):
    try:
        quoter = web3.eth.contract(address=Web3.to_checksum_address(QUOTER_V2_ADDRESS), abi=QUOTER_V2_ABI)
        amount_in = 10000 * 10**TOKEN0_DECIMALS  # 10 000 USDC
        result = quoter.functions.quoteExactInputSingle({
            'tokenIn': Web3.to_checksum_address(TOKEN0_ADDRESS),
            'tokenOut': Web3.to_checksum_address(TOKEN1_ADDRESS),
        ...
        comp_received = mp.mpf(result[0]) / 10**TOKEN1_DECIMALS
        expected_comp = mp.mpf(10000) / price_usdc_per_comp
        return float(1 - comp_received / expected_comp)
    except Exception as e:
    ...
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                usdc_amount, comp_amount, sqrtPriceX96, liquidity, tick, amount0_raw, amount1_raw = decode_swap_event(row['data'])
        ...
        # Passe 2 : TVL, slip_1k et slip_10k par bloc unique (minimise les appels RPC)
