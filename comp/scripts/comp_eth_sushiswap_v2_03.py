...
# SushiSwap V2 = Uniswap V2 fork — même topic0 pour l'event Swap
EXPECTED_TOPIC0 = "0xd78ad95fa46c994b6551d0da85fc275fe613ce37657fb8d5e3d130840159d822"

POOL_ADDRESS    = "0x31503dCb60119A812feE820bb7042752019F2355"
TOKEN0_ADDRESS  = "0xc00e94Cb662C3520282E6f5717214004A7f26888"  # COMP
...
SCHEMA_VERSION          = "dex_sushiswap_v2_v1"
DEX_PROTOCOL            = "sushiswap"
DEX_VERSION             = "v2"
BASE_TOKEN_ADDRESS      = TOKEN0_ADDRESS   # COMP
QUOTE_TOKEN_ADDRESS     = TOKEN1_ADDRESS   # WETH
BASE_TOKEN_SYMBOL       = "COMP"
...
def decode_swap_event(data_hex):
    try:
        ...
        net0 = mp.mpf(amount0In - amount0Out) / 10**TOKEN0_DECIMALS  # COMP
...
def calculate_price(net0_comp, net1_weth):
    try:
        if net0_comp == 0:
            raise ValueError("COMP amount est zéro")
        price_eth_per_comp = abs(net1_weth) / abs(net0_comp)
        volume_eth = abs(net1_weth)
        return price_eth_per_comp, volume_eth
    except Exception as e:
...
def get_tvl_at_block(web3, block_number, price_eth_per_comp):
    try:
        reserve0, reserve1 = get_reserves(web3, block_number)
        bal0 = mp.mpf(reserve0) / 10**TOKEN0_DECIMALS  # COMP
        bal1 = mp.mpf(reserve1) / 10**TOKEN1_DECIMALS  # WETH
        # TVL exprimé en ETH
        return float(bal1 + bal0 * price_eth_per_comp)
    except Exception as e:
        print(f"Erreur TVL au bloc {block_number}: {e}")
        return None


def get_slip_1k(web3, block_number, price_eth_per_comp):
    try:
        reserve0, reserve1 = get_reserves(web3, block_number)
        # Simulation: 1 ETH → COMP (tokenIn=WETH=token1, tokenOut=COMP=token0)
        amount_in = mp.mpf(1) * 10**TOKEN1_DECIMALS
        amount_out = (997 * amount_in * reserve0) / (1000 * reserve1 + 997 * amount_in)
        comp_received = amount_out / 10**TOKEN0_DECIMALS
        expected_comp = mp.mpf(1) / price_eth_per_comp
...
def get_slip_10k(web3, block_number, price_eth_per_comp):
    try:
        reserve0, reserve1 = get_reserves(web3, block_number)
        amount_in = mp.mpf(10) * 10**TOKEN1_DECIMALS  # 10 WETH/ETH
        amount_out = (997 * amount_in * reserve0) / (1000 * reserve1 + 997 * amount_in)
        comp_received = amount_out / 10**TOKEN0_DECIMALS
        expected_comp = mp.mpf(10) / price_eth_per_comp
        return float(1 - comp_received / expected_comp)
    except Exception as e:
...
def process_swap_logs(csv_path, web3):
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                if log_address and log_address != POOL_ADDRESS.lower():
                    continue
                net0_comp, net1_weth, amount0_raw, amount1_raw = decode_swap_event(row['data'])
                price, volume = calculate_price(net0_comp, net1_weth)
                ...
                rows.append({
                    'timestamp':           timestamp,
                    'price_eth_per_comp':  price,
                    'eth_amount':          net1_weth,
                    'comp_amount':         net0_comp,
                    'volume_eth':          volume,
                    'block_number':        bn,
        ...
        print(f"\nCal réserves, TVL, slip_1k et slip_10k pour {len(unique_blocks)} blocs uniques...")
        reserve_cache = {}
        tvl_cache     = {}
        slip_cache    = {}
        slip10k_cache = {}
        ...
        print(result_df['price_eth_per_comp'].describe())
        return result_df
    except Exception as e:
...
def main(output_filename='comp_eth_sushiswap_v2_03_last.csv'):
    if not csv_files:
