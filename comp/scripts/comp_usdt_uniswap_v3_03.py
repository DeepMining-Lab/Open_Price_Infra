...
POOL_ADDRESS    = "0x68c5ea31aefe12713642aec96999df9319942641"
...
def decode_swap_event(data_hex):
    try:
        ...
        comp_amount  = mp.mpf(amount0) / 10**TOKEN0_DECIMALS
        usdt_amount  = mp.mpf(amount1) / 10**TOKEN1_DECIMALS
        return usdt_amount, comp_amount, sqrtPriceX96, liquidity, tick, amount0, amount1
...
def calculate_price(sqrtPriceX96, comp_amount, usdt_amount):
    try:
        sqrt_price = mp.mpf(sqrtPriceX96) / (2 ** 96)
        # COMP est token0, USDT est token1 → prix = sqrtP² × 1e12
        price_usdt_per_comp = sqrt_price ** 2 * mp.mpf("1e12")
        volume_usdt = abs(usdt_amount)
        return price_usdt_per_comp, volume_usdt
    except Exception as e:
...
def get_tvl_at_block(web3, block_number, price_usdt_per_comp):
    try:
        ...
        bal0 = mp.mpf(raw0) / 10**TOKEN0_DECIMALS  # COMP
        bal1 = mp.mpf(raw1) / 10**TOKEN1_DECIMALS  # USDT ≈ 1 USD
        return float(bal1 + bal0 * price_usdt_per_comp)
    except Exception as e:
        print(f"Erreur TVL au bloc {block_number}: {e}")
        return None


def get_slip_1k(web3, block_number, price_usdt_per_comp):
    try:
        ...
        comp_received = mp.mpf(result[0]) / 10**TOKEN0_DECIMALS
        expected_comp = mp.mpf(1000) / price_usdt_per_comp
...
def get_slip_10k(web3, block_number, price_usdt_per_comp):
    try:
        quoter = web3.eth.contract(address=Web3.to_checksum_address(QUOTER_V2_ADDRESS), abi=QUOTER_V2_ABI)
        amount_in = 10000 * 10**TOKEN1_DECIMALS  # 10 000 USDT (token1)
        result = quoter.functions.quoteExactInputSingle({
            'tokenIn': Web3.to_checksum_address(TOKEN1_ADDRESS),
            'tokenOut': Web3.to_checksum_address(TOKEN0_ADDRESS),
            'amountIn': amount_in, 'fee': POOL_FEE_TIER, 'sqrtPriceLimitX96': 0,
        }).call(block_identifier=block_number)
        comp_received = mp.mpf(result[0]) / 10**TOKEN0_DECIMALS
        expected_comp = mp.mpf(10000) / price_usdt_per_comp
        return float(1 - comp_received / expected_comp)
    except Exception as e:
...
def process_uniswap_logs(csv_path, web3):
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                if log_address and log_address != POOL_ADDRESS.lower():
                    continue
                usdt_amount, comp_amount, sqrtPriceX96, liquidity, tick, amount0_raw, amount1_raw = decode_swap_event(row['data'])
                price, volume = calculate_price(sqrtPriceX96, comp_amount, usdt_amount)
        ...
        print(result_df['price_usdt_per_comp'].describe())
        return result_df
    except Exception as e:
...
def main(output_filename='comp_usdt_uniswap_v3_03_last.csv'):
    if not csv_files:
