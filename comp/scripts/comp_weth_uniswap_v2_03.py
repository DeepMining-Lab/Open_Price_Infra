...
def calculate_price(net0_comp, net1_weth):
    try:
        ...
        price_weth_per_comp = abs(net1_weth) / abs(net0_comp)
        volume_weth = abs(net1_weth)
...
def get_slip_1k(web3, block_number, price_weth_per_comp):
    try:
        reserve0, reserve1 = get_reserves(web3, block_number)
        # Simulation: 1 WETH → COMP (tokenIn=WETH=token1, tokenOut=COMP=token0)
...
def get_slip_10k(web3, block_number, price_weth_per_comp):
    try:
        reserve0, reserve1 = get_reserves(web3, block_number)
        amount_in = mp.mpf(10) * 10**TOKEN1_DECIMALS  # 10 WETH
        amount_out = (997 * amount_in * reserve0) / (1000 * reserve1 + 997 * amount_in)
        comp_received = amount_out / 10**TOKEN0_DECIMALS
...
def main(output_filename='comp_weth_uniswap_v2_03_last.csv'):
    if not csv_files:
