...
POOL_ADDRESS    = "0xea4ba4ce14fdd287f380b55419b1c5b6c3f22ab6"
...
def decode_swap_event(data_hex):
    try:
        ...
        return weth_amount, comp_amount, sqrtPriceX96, liquidity, tick, amount0, amount1
...
def calculate_price(sqrtPriceX96, comp_amount, weth_amount):
    try:
        sqrt_price = mp.mpf(sqrtPriceX96) / (2 ** 96)
        # COMP est token0, WETH est token1 → prix = sqrtP² (les deux à 18 dec)
        price_weth_per_comp = sqrt_price ** 2
        volume_weth = abs(weth_amount)
        return price_weth_per_comp, volume_weth
    except Exception as e:
...
def get_tvl_at_block(web3, block_number, price_weth_per_comp):
    try:
        ...
        # TVL exprimé en WETH
        return float(bal1 + bal0 * price_weth_per_comp)
    except Exception as e:
        print(f"Erreur TVL au bloc {block_number}: {e}")
        return None


def get_slip_1k(web3, block_number, price_weth_per_comp):
    try:
        quoter    = web3.eth.contract(address=Web3.to_checksum_address(QUOTER_V2_ADDRESS), abi=QUOTER_V2_ABI)
        # Simulation d'un achat de COMP avec 1 WETH
        ...
        expected_comp = mp.mpf(1) / price_weth_per_comp
...
def get_slip_10k(web3, block_number, price_weth_per_comp):
    try:
        quoter = web3.eth.contract(address=Web3.to_checksum_address(QUOTER_V2_ADDRESS), abi=QUOTER_V2_ABI)
        ...
        comp_received = mp.mpf(result[0]) / 10**TOKEN0_DECIMALS
        expected_comp = mp.mpf(10) / price_weth_per_comp
        return float(1 - comp_received / expected_comp)
    except Exception as e:
...
def process_uniswap_logs(csv_path, web3):
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                if log_address and log_address != POOL_ADDRESS.lower():
                    continue
                weth_amount, comp_amount, sqrtPriceX96, liquidity, tick, amount0_raw, amount1_raw = decode_swap_event(row['data'])
                price, volume = calculate_price(sqrtPriceX96, comp_amount, weth_amount)
        ...
        print(result_df['price_weth_per_comp'].describe())
        return result_df
    except Exception as e:
...
def main(output_filename='comp_weth_uniswap_v3_03_last.csv'):
    if not csv_files:
