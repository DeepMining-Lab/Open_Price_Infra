...
POOL_ADDRESS    = "0x5ab53EE1d50eeF2C1DD3D5402789cd27bB52c1bB"
...
def decode_swap_event(data_hex):
    try:
        ...
        aave_amount  = mp.mpf(amount0) / 10**TOKEN0_DECIMALS
        weth_amount  = mp.mpf(amount1) / 10**TOKEN1_DECIMALS
        return aave_amount, weth_amount, sqrtPriceX96, liquidity, tick, amount0, amount1
...
def calculate_price(sqrtPriceX96, aave_amount, weth_amount):
    try:
        sqrt_price = mp.mpf(sqrtPriceX96) / (2 ** 96)
        # AAVE est token0, WETH est token1 → prix = sqrtP² (les deux à 18 dec)
        price_weth_per_aave = sqrt_price ** 2
        volume_weth = abs(weth_amount)
        return price_weth_per_aave, volume_weth
    except Exception as e:
...
def get_tvl_at_block(web3, block_number, price_weth_per_aave):
    try:
        ...
        bal0 = mp.mpf(raw0) / 10**TOKEN0_DECIMALS  # AAVE
        bal1 = mp.mpf(raw1) / 10**TOKEN1_DECIMALS  # WETH
        # TVL exprimé en WETH
        return float(bal1 + bal0 * price_weth_per_aave)
    except Exception as e:
        print(f"Erreur TVL au bloc {block_number}: {e}")
        return None


def get_slip_1k(web3, block_number, price_weth_per_aave):
    try:
        quoter    = web3.eth.contract(address=Web3.to_checksum_address(QUOTER_V2_ADDRESS), abi=QUOTER_V2_ABI)
        # Simulation d'un achat de AAVE avec 1 WETH
        ...
        expected_aave = mp.mpf(1) / price_weth_per_aave
...
def get_slip_10k(web3, block_number, price_weth_per_aave):
    try:
        quoter    = web3.eth.contract(address=Web3.to_checksum_address(QUOTER_V2_ADDRESS), abi=QUOTER_V2_ABI)
        # Simulation d'un achat de AAVE avec 10 WETH
        amount_in = 10 * 10**TOKEN1_DECIMALS
        result    = quoter.functions.quoteExactInputSingle({
            'tokenIn':           Web3.to_checksum_address(TOKEN1_ADDRESS),
            'tokenOut':          Web3.to_checksum_address(TOKEN0_ADDRESS),
            'amountIn':          amount_in,
        ...
        aave_received = mp.mpf(result[0]) / 10**TOKEN0_DECIMALS
        expected_aave = mp.mpf(10) / price_weth_per_aave
        return float(1 - aave_received / expected_aave)
    except Exception as e:
...
def process_uniswap_logs(csv_path, web3):
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                if log_address and log_address != POOL_ADDRESS.lower():
                    continue
                aave_amount, weth_amount, sqrtPriceX96, liquidity, tick, amount0_raw, amount1_raw = decode_swap_event(row['data'])
                price, volume = calculate_price(sqrtPriceX96, aave_amount, weth_amount)
        ...
        print(result_df['price_weth_per_aave'].describe())
        return result_df
    except Exception as e:
...
def main(output_filename='aave_weth_uniswap_v3_03_last.csv'):
    if not csv_files:
