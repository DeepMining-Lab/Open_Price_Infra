...
DATA_FILE_UNISWAP="$PROJECT_DIR/data/uniswap_aave_usd.csv"
DATA_FILE_CHAINLINK="$PROJECT_DIR/data/chainlink_aave_usd.csv"
OUTPUT_DIR="$PROJECT_DIR/data/output"
LAST_FILE_UNISWAP="$PROJECT_DIR/data/uniswap_aave_usd_last.csv"
...
for i in $(seq 1 $MAX); do
  ...
  if scp data/{chainlink_aave_usd.csv,uniswap_aave_usd.csv} debian@extract.lan.text-analytics.ch:/data/ethereum/prices; then
    SCP_OK=true
    break
  fi
