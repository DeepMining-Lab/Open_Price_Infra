...
POOL_ADDRESS    = "0x4d1AD4a9E61BC0E5529d64f38199cCFca56F5A42"
...
def decode_swap_event(data_hex):
    try:
        ...
        return aave_amount, usdt_amount, sqrtPriceX96, liquidity, tick, amount0, amount1
...
def calculate_price(sqrtPriceX96, aave_amount, usdt_amount):
    try:
        sqrt_price = mp.mpf(sqrtPriceX96) / (2 ** 96)
        # AAVE est token0, USDT est token1 → prix = sqrtP² × 1e12
        price_usdt_per_aave = sqrt_price ** 2 * mp.mpf("1e12")
        volume_usdt = abs(usdt_amount)
        return price_usdt_per_aave, volume_usdt
    except Exception as e:
...
def get_tvl_at_block(web3, block_number, price_usdt_per_aave):
    try:
        ...
        return float(bal1 + bal0 * price_usdt_per_aave)
    except Exception as e:
        print(f"Erreur TVL au bloc {block_number}: {e}")
        return None


def get_slip_1k(web3, block_number, price_usdt_per_aave):
    try:
        ...
        expected_aave = mp.mpf(1000) / price_usdt_per_aave
...
def get_slip_10k(web3, block_number, price_usdt_per_aave):
    try:
        quoter    = web3.eth.contract(address=Web3.to_checksum_address(QUOTER_V2_ADDRESS), abi=QUOTER_V2_ABI)
        amount_in = 10000 * 10**TOKEN1_DECIMALS
        result    = quoter.functions.quoteExactInputSingle({
            'tokenIn':           Web3.to_checksum_address(TOKEN1_ADDRESS),
            'tokenOut':          Web3.to_checksum_address(TOKEN0_ADDRESS),
            'amountIn':          amount_in,
        ...
        aave_received = mp.mpf(result[0]) / 10**TOKEN0_DECIMALS
        expected_aave = mp.mpf(10000) / price_usdt_per_aave
        return float(1 - aave_received / expected_aave)
    except Exception as e:
...
def process_uniswap_logs(csv_path, web3):
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                if log_address and log_address != POOL_ADDRESS.lower():
                    continue
                aave_amount, usdt_amount, sqrtPriceX96, liquidity, tick, amount0_raw, amount1_raw = decode_swap_event(row['data'])
                price, volume = calculate_price(sqrtPriceX96, aave_amount, usdt_amount)
        ...
        print(result_df['price_usdt_per_aave'].describe())
        return result_df
    except Exception as e:
...
def main(output_filename='aave_usdt_uniswap_v3_03_last.csv'):
    if not csv_files:
