...
def decode_swap_event(data_hex):
    try:
        ...
        sqrtPriceX96 = int(sqrtPriceX96_hex, 16)
                aave_amount = mp.mpf(amount0) / 10**18   # AAVE a 18 décimales (token0)
        usdc_amount  = mp.mpf(amount1) / 10**6    # USDC a 6 décimales (token1)
        return usdc_amount, aave_amount, sqrtPriceX96
...
def calculate_price(sqrtPriceX96, aave_amount, usdc_amount):
    try:
        ...
        # sqrtP = sqrt(USDC_raw / AAVE_raw) → price_usdc_per_aave_raw = sqrt_price²
        # Ajustement décimales : AAVE=18 dec, USDC=6 dec → ×1e12
        price_usdc_per_aave = sqrt_price ** 2 * mp.mpf("1e12")
        volume_usdc = abs(usdc_amount) + abs(aave_amount * price_usdc_per_aave)
...
def process_uniswap_logs(csv_path, web3):
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                if row['topic0'] != EXPECTED_TOPIC0:
                    ...
                    continue
                usdc_amount, aave_amount, sqrtPriceX96 = decode_swap_event(row['data'])
                ...
                if timestamp:
                    ...
                    aave_amounts.append(aave_amount)
        ...
        result_df = pd.DataFrame({
            ...
            'price_usdc_per_aave':  prices,
            'usdc_amount':           usdc_amounts,
            'aave_amount':          aave_amounts,
            'volume_usdc':           volumes,
...
def main(output_filename='uniswap_aave_usd_last.csv'):
    if not csv_files:
