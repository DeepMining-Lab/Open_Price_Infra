...
def decode_swap_event(data_hex):
    try:
        ...
        net0 = mp.mpf(amount0In - amount0Out) / 10**TOKEN0_DECIMALS  # WETH
        net1 = mp.mpf(amount1In - amount1Out) / 10**TOKEN1_DECIMALS  # USDT
        return net0, net1
        ...
        for index, row in df.iterrows():
            try:
                ...
                if log_address and log_address != POOL_ADDRESS.lower():
                    continue
                net0_weth, net1_usdt = decode_swap_event(row['data'])
