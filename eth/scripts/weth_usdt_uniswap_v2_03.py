...
def get_all_metrics(web3, bn, price_ref):
    ...
    try:
        r0, r1 = get_reserves(web3, bn)  # r0=WETH, r1=USDT

        bal0 = mp.mpf(r0) / 10**TOKEN0_DECIMALS  # WETH
        bal1 = mp.mpf(r1) / 10**TOKEN1_DECIMALS  # USDT
        tvl = float(bal1 + bal0 * price_ref)  # TVL en USDT

        # slip_1k: 1 000 USDT → WETH (tokenIn=USDT=token1, tokenOut=WETH=token0)
        ain_1k  = mp.mpf(1000)  * 10**TOKEN1_DECIMALS
        aout_1k = (997 * ain_1k * r0) / (1000 * r1 + 997 * ain_1k)
        slip1k  = float(1 - (aout_1k / 10**TOKEN0_DECIMALS) / (mp.mpf(1000)  / price_ref))

        # slip_10k: 10 000 USDT → WETH
        ain_10k  = mp.mpf(10000) * 10**TOKEN1_DECIMALS
        aout_10k = (997 * ain_10k * r0) / (1000 * r1 + 997 * ain_10k)
        slip10k  = float(1 - (aout_10k / 10**TOKEN0_DECIMALS) / (mp.mpf(10000) / price_ref))
