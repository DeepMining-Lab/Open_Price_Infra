...
POOL_ADDRESS    = "0xC40D16476380e4037e6b1A2594cAF6a6cc8Da967"
...
def decode_swap_event(data_hex):
    try:
        ...
        net0 = mp.mpf(amount0In - amount0Out) / 10**TOKEN0_DECIMALS  # LINK
...
def calculate_price(net0_link, net1_weth):
    try:
        if net0_link == 0:
            raise ValueError("LINK amount est zéro")
        price_eth_per_link = abs(net1_weth) / abs(net0_link)
        volume_eth = abs(net1_weth)
        return price_eth_per_link, volume_eth
    except Exception as e:
...
def get_tvl_at_block(web3, block_number, price_eth_per_link):
    try:
        reserve0, reserve1 = get_reserves(web3, block_number)
        bal0 = mp.mpf(reserve0) / 10**TOKEN0_DECIMALS  # LINK
        bal1 = mp.mpf(reserve1) / 10**TOKEN1_DECIMALS  # WETH
        # TVL exprimé en ETH
        return float(bal1 + bal0 * price_eth_per_link)
    except Exception as e:
        print(f"Erreur TVL au bloc {block_number}: {e}")
        return None


def get_slip_1k(web3, block_number, price_eth_per_link):
    try:
        reserve0, reserve1 = get_reserves(web3, block_number)
        # Simulation: 1 ETH → LINK (tokenIn=WETH=token1, tokenOut=LINK=token0)
        amount_in = mp.mpf(1) * 10**TOKEN1_DECIMALS
        amount_out = (997 * amount_in * reserve0) / (1000 * reserve1 + 997 * amount_in)
        link_received = amount_out / 10**TOKEN0_DECIMALS
        expected_link = mp.mpf(1) / price_eth_per_link
...
def get_slip_10k(web3, block_number, price_eth_per_link):
    try:
        reserve0, reserve1 = get_reserves(web3, block_number)
        amount_in = mp.mpf(10) * 10**TOKEN1_DECIMALS  # 10 ETH
        amount_out = (997 * amount_in * reserve0) / (1000 * reserve1 + 997 * amount_in)
        link_received = amount_out / 10**TOKEN0_DECIMALS
        expected_link = mp.mpf(10) / price_eth_per_link
        return float(1 - link_received / expected_link)
    except Exception as e:
...
def process_swap_logs(csv_path, web3):
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                if log_address and log_address != POOL_ADDRESS.lower():
                    continue
                net0_link, net1_weth, amount0_raw, amount1_raw = decode_swap_event(row['data'])
                price, volume = calculate_price(net0_link, net1_weth)
                ...
                rows.append({
                    'timestamp':          timestamp,
                    'price_eth_per_link': price,
                    'eth_amount':         net1_weth,
                    'link_amount':        net0_link,
                    'volume_eth':         volume,
                    'block_number':       bn,
        ...
        print(result_df['price_eth_per_link'].describe())
        return result_df
    except Exception as e:
...
def main(output_filename='link_eth_sushiswap_v2_03_last.csv'):
    if not csv_files:
