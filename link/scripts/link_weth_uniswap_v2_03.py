...
POOL_ADDRESS    = "0xa2107fa5B38d9BbD2C461D6EDf11B11A50F6b974"
...
def get_slip_1k(web3, block_number, price_weth_per_link):
    try:
        reserve0, reserve1 = get_reserves(web3, block_number)
        # Simulation: 1 WETH → LINK (tokenIn=WETH=token1, tokenOut=LINK=token0)
...
def get_slip_10k(web3, block_number, price_weth_per_link):
    try:
        reserve0, reserve1 = get_reserves(web3, block_number)
        amount_in = mp.mpf(10) * 10**TOKEN1_DECIMALS  # 10 WETH
        amount_out = (997 * amount_in * reserve0) / (1000 * reserve1 + 997 * amount_in)
        link_received = amount_out / 10**TOKEN0_DECIMALS
        expected_link = mp.mpf(10) / price_weth_per_link
        return float(1 - link_received / expected_link)
    except Exception as e:
