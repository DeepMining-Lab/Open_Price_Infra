...
POOL_ADDRESS    = Web3.to_checksum_address("0xa2107fa5b38d9bbd2c461d6edf11b11a50f6b974")
...
def calculate_price(net0_link, net1_weth):
    try:
        ...
        price_weth_per_link = abs(net1_weth) / abs(net0_link)
...
def get_all_metrics(web3, bn, price_ref):
    ...
    try:
        ...
        tvl = float(bal1 + bal0 * price_ref)      # TVL en WETH

        # slip_1k: 1 WETH → LINK (tokenIn=WETH=token1, tokenOut=LINK=token0)
...
def main(output_filename='link_weth_uniswap_v2_03_last.csv'):
    if not csv_files:
