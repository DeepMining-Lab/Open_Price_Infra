...
DATA_FILE_UNISWAP="$PROJECT_DIR/data/uniswap_link_usd.csv"
...
for i in $(seq 1 $MAX); do
  ...
  if scp data/{chainlink_link_usd.csv,uniswap_link_usd.csv} debian@extract.lan.text-analytics.ch:/data/ethereum/prices; then
    SCP_OK=true
    break
  fi
