...
POOL_ADDRESS    = "0xA5F43B0EBaEfbEd5B1f1bFc809AF15254EA1e9c4"
...
def decode_swap_event(data_hex):
    try:
        ...
        return eth_amount, link_amount, sqrtPriceX96, liquidity, tick, amount0, amount1
...
def get_slip_1k(web3, block_number, price_eth_per_link):
    try:
        ...
        # Simulation d'un achat de LINK avec 1 ETH
        ...
        for index, row in df.iterrows():
            try:
                ...
                if log_address and log_address != POOL_ADDRESS.lower():
                    continue
                eth_amount, link_amount, sqrtPriceX96, liquidity, tick, amount0_raw, amount1_raw = decode_swap_event(row['data'])
                price, volume = calculate_price(sqrtPriceX96, link_amount, eth_amount)
...
def main(output_filename='link_eth_sushiswap_v3_03_last.csv'):
    if not csv_files:
