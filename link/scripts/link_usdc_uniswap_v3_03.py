...
def decode_swap_event(data_hex):
    try:
        ...
        link_amount  = mp.mpf(amount0) / 10**TOKEN0_DECIMALS  # LINK
        usdc_amount  = mp.mpf(amount1) / 10**TOKEN1_DECIMALS  # USDC
        return usdc_amount, link_amount, sqrtPriceX96, liquidity, tick, amount0, amount1
...
def calculate_price(sqrtPriceX96, link_amount, usdc_amount):
    try:
        ...
        # LINK est token0, USDC est token1 → prix = sqrtP² × 1e12
...
def get_tvl_at_block(web3, block_number, price_usdc_per_link):
    try:
        ...
        return float(bal1 + bal0 * price_usdc_per_link)
    except Exception as e:
        print(f"Erreur TVL au bloc {block_number}: {e}")
        return None


def get_slip_1k(web3, block_number, price_usdc_per_link):
    try:
        ...
        expected_link  = mp.mpf(1000) / price_usdc_per_link
...
def get_slip_10k(web3, block_number, price_usdc_per_link):
    try:
        quoter    = web3.eth.contract(
            address=Web3.to_checksum_address(QUOTER_V2_ADDRESS),
            abi=QUOTER_V2_ABI,
        ...
        link_received  = mp.mpf(result[0]) / 10**TOKEN0_DECIMALS
        expected_link  = mp.mpf(10000) / price_usdc_per_link
        return float(1 - link_received / expected_link)
    except Exception as e:
    ...
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                usdc_amount, link_amount, sqrtPriceX96, liquidity, tick, amount0_raw, amount1_raw = decode_swap_event(row['data'])
...
def main(output_filename='link_usdc_uniswap_v3_03_last.csv'):
    if not csv_files:
