...
def decode_swap_event(data_hex):
    try:
        ...
        return weth_amount, uni_amount, sqrtPriceX96, liquidity, tick, amount0, amount1
...
def get_slip_1k(web3, block_number, price_weth_per_uni):
    try:
        ...
        # Simulation d'un achat de UNI avec 1 WETH
        ...
        for index, row in df.iterrows():
            try:
                ...
                if log_address and log_address != POOL_ADDRESS.lower():
                    continue
                weth_amount, uni_amount, sqrtPriceX96, liquidity, tick, amount0_raw, amount1_raw = decode_swap_event(row['data'])
                price, volume = calculate_price(sqrtPriceX96, uni_amount, weth_amount)
...
def main(output_filename='uni_weth_uniswap_v3_03_last.csv'):
    if not csv_files:
