...
def decode_swap_event(data_hex):
    try:
        ...
        uni_amount = mp.mpf(amount0) / 10**18   # UNI a 18 décimales (token0)
        usdc_amount  = mp.mpf(amount1) / 10**6    # USDC a 6 décimales (token1)
        return usdc_amount, uni_amount, sqrtPriceX96
...
def calculate_price(sqrtPriceX96, uni_amount, usdc_amount):
    try:
        ...
        # sqrtP = sqrt(USDC_raw / UNI_raw) → price_usdc_per_uni_raw = sqrt_price²
        # Ajustement décimales : UNI=18 dec, USDC=6 dec → ×1e12
        price_usdc_per_uni = sqrt_price ** 2 * mp.mpf("1e12")
        volume_usdc = abs(usdc_amount) + abs(uni_amount * price_usdc_per_uni)
...
def process_uniswap_logs(csv_path, web3):
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                if row['topic0'] != EXPECTED_TOPIC0:
                    ...
                    continue
                usdc_amount, uni_amount, sqrtPriceX96 = decode_swap_event(row['data'])
        ...
        result_df = pd.DataFrame({
            ...
            'price_usdc_per_uni':  prices,
            'usdc_amount':           usdc_amounts,
            'uni_amount':          uni_amounts,
            'volume_usdc':           volumes,
