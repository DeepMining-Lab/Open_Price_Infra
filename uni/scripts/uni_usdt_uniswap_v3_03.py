...
POOL_ADDRESS    = "0x3470447f3cECFfaC709D3E783A307790B0208d60"
...
def decode_swap_event(data_hex):
    try:
        ...
        return usdt_amount, uni_amount, sqrtPriceX96, liquidity, tick, amount0, amount1
...
def calculate_price(sqrtPriceX96, uni_amount, usdt_amount):
    try:
        sqrt_price = mp.mpf(sqrtPriceX96) / (2 ** 96)
        # UNI est token0, USDT est token1 → prix = sqrtP² × 1e12
        price_usdt_per_uni = sqrt_price ** 2 * mp.mpf("1e12")
        volume_usdt = abs(usdt_amount)
        return price_usdt_per_uni, volume_usdt
    except Exception as e:
...
def get_tvl_at_block(web3, block_number, price_usdt_per_uni):
    try:
        ...
        return float(bal1 + bal0 * price_usdt_per_uni)
    except Exception as e:
        print(f"Erreur TVL au bloc {block_number}: {e}")
        return None


def get_slip_1k(web3, block_number, price_usdt_per_uni):
    try:
        ...
        expected_uni = mp.mpf(1000) / price_usdt_per_uni
...
def get_slip_10k(web3, block_number, price_usdt_per_uni):
    try:
        quoter    = web3.eth.contract(address=Web3.to_checksum_address(QUOTER_V2_ADDRESS), abi=QUOTER_V2_ABI)
        ...
        uni_received = mp.mpf(result[0]) / 10**TOKEN0_DECIMALS
        expected_uni = mp.mpf(10000) / price_usdt_per_uni
        return float(1 - uni_received / expected_uni)
    except Exception as e:
...
def process_uniswap_logs(csv_path, web3):
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                if log_address and log_address != POOL_ADDRESS.lower():
                    continue
                usdt_amount, uni_amount, sqrtPriceX96, liquidity, tick, amount0_raw, amount1_raw = decode_swap_event(row['data'])
                price, volume = calculate_price(sqrtPriceX96, uni_amount, usdt_amount)
        ...
        print(result_df['price_usdt_per_uni'].describe())
        return result_df
    except Exception as e:
...
def main(output_filename='uni_usdt_uniswap_v3_03_last.csv'):
    if not csv_files:
