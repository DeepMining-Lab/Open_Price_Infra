...
POOL_ADDRESS    = "0xEd35BD3D709CB50f34113A4962607B2A0F93cD92"
...
# SushiSwap V3 QuoterV2
QUOTER_V2_ADDRESS = "0x64e8802FE490fa7cc61d3463958199161Bb608A7"
QUOTER_V2_ABI = [
    {
        "inputs": [
            {
...
SCHEMA_VERSION          = "dex_sushiswap_v3_v1"
...
def decode_swap_event(data_hex):
    try:
        ...
        uni_amount   = mp.mpf(amount0) / 10**TOKEN0_DECIMALS
        eth_amount   = mp.mpf(amount1) / 10**TOKEN1_DECIMALS
        return eth_amount, uni_amount, sqrtPriceX96, liquidity, tick, amount0, amount1
...
def calculate_price(sqrtPriceX96, uni_amount, eth_amount):
    try:
        sqrt_price = mp.mpf(sqrtPriceX96) / (2 ** 96)
        # UNI est token0, WETH est token1 → prix = sqrtP² (les deux à 18 dec)
        price_eth_per_uni = sqrt_price ** 2
        volume_eth = abs(eth_amount)
        return price_eth_per_uni, volume_eth
    except Exception as e:
...
def get_tvl_at_block(web3, block_number, price_eth_per_uni):
    try:
...
def get_slip_1k(web3, block_number, price_eth_per_uni):
    try:
        ...
        expected_uni = mp.mpf(1) / price_eth_per_uni
...
def get_slip_10k(web3, block_number, price_eth_per_uni):
    try:
        quoter    = web3.eth.contract(address=Web3.to_checksum_address(QUOTER_V2_ADDRESS), abi=QUOTER_V2_ABI)
        amount_in = 10 * 10**TOKEN1_DECIMALS  # 10 ETH
        result    = quoter.functions.quoteExactInputSingle({
            'tokenIn':           Web3.to_checksum_address(TOKEN1_ADDRESS),
            'tokenOut':          Web3.to_checksum_address(TOKEN0_ADDRESS),
            'amountIn':          amount_in,
        ...
        uni_received = mp.mpf(result[0]) / 10**TOKEN0_DECIMALS
        expected_uni = mp.mpf(10) / price_eth_per_uni
        return float(1 - uni_received / expected_uni)
    except Exception as e:
...
def process_uniswap_logs(csv_path, web3):
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                if log_address and log_address != POOL_ADDRESS.lower():
                    continue
                eth_amount, uni_amount, sqrtPriceX96, liquidity, tick, amount0_raw, amount1_raw = decode_swap_event(row['data'])
                price, volume = calculate_price(sqrtPriceX96, uni_amount, eth_amount)
        ...
        print(result_df['price_eth_per_uni'].describe())
        return result_df
    except Exception as e:
...
def main(output_filename='uni_eth_sushiswap_v3_03_last.csv'):
    if not csv_files:
