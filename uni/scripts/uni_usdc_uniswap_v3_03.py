...
def calculate_price(sqrtPriceX96, uni_amount, usdc_amount):
    try:
        sqrt_price = mp.mpf(sqrtPriceX96) / (2 ** 96)
        # UNI est token0, USDC est token1 → prix = sqrtP² × 1e12
        price_usdc_per_uni = sqrt_price ** 2 * mp.mpf("1e12")
        volume_usdc = abs(usdc_amount)
        return price_usdc_per_uni, volume_usdc
    except Exception as e:
...
def get_tvl_at_block(web3, block_number, price_usdc_per_uni):
    try:
        ...
        bal0 = mp.mpf(raw0) / 10**TOKEN0_DECIMALS  # UNI, valorisé en USD
        bal1 = mp.mpf(raw1) / 10**TOKEN1_DECIMALS  # USDC ≈ 1 USD
        return float(bal1 + bal0 * price_usdc_per_uni)
    except Exception as e:
        print(f"Erreur TVL au bloc {block_number}: {e}")
        return None


def get_slip_1k(web3, block_number, price_usdc_per_uni):
    try:
        ...
        expected_uni  = mp.mpf(1000) / price_usdc_per_uni
...
def get_slip_10k(web3, block_number, price_usdc_per_uni):
    try:
        quoter    = web3.eth.contract(
            address=Web3.to_checksum_address(QUOTER_V2_ADDRESS),
            abi=QUOTER_V2_ABI,
        ...
        uni_received  = mp.mpf(result[0]) / 10**TOKEN0_DECIMALS
        expected_uni  = mp.mpf(10000) / price_usdc_per_uni
        return float(1 - uni_received / expected_uni)
    except Exception as e:
...
def process_uniswap_logs(csv_path, web3):
    try:
        ...
        print("\nAperçu des prix:")
        print(result_df['price_usdc_per_uni'].describe())
        return result_df
...
def main(output_filename='uni_usdc_uniswap_v3_03_last.csv'):
    if not csv_files:
